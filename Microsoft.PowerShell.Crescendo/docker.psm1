...
Function Get-DockerContainer
{
    ...
    BEGIN
    {
        ...
        $__outputHandlers = @{
            Default = @{ StreamOutput = $False; Handler = { if ($args)
                    {
    ...
    PROCESS
    {
        ...
        foreach ($paramName in $PSBoundParameters.Keys | Sort-Object { $__PARAMETERMAP[$_].OriginalPosition })
        {
            ...
            if ($param)
            {
                if ( $value -is [switch] )
                {
                    $__commandArgs += if ( $value.IsPresent )
                    {
                        $param.OriginalName 
                    }
                    else
                    {
                        $param.DefaultMissingValue 
                    } 
                }
                elseif ( $param.NoGap )
                {
                    $__commandArgs += "{0}""{1}""" -f $param.OriginalName, $value 
                }
                else
                {
                    $__commandArgs += $param.OriginalName; $__commandArgs += $value | ForEach-Object { $_ }
                }
            }
        }
...
Original Command: docker ps


.EXAMPLE
PS> Get-DockerContainer -Force

List all containers
Original Command: docker ps -a


.EXAMPLE
PS> Get-DockerContainer -Force -Filter

Filter output based on conditions provided
Original Command: docker ps -a -f
